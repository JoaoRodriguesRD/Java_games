...
public class AStar {
	...
	public static List<Node> findPath(World world, Vector2i start, Vector2i end) {
		...
		while (openList.size() > 0) {
			...
			if(current.tile.equals(end)) {//chegou no ponto final, basta retornar o valor do caminho
			...
			for(int  i = 0; i < 9;i++) {// verificar as 8 posicoes proximas do inimigo
				if(i == 4) {//posicao do inimigo, nao precisa verificar
					continue;
				}
				...
				if(i == 0){// esses if servem para andar nas diagonais
		...
		closedList.clear();//nao encontrou caminho
		return null;
	}
